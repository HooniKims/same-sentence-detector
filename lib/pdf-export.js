...
export async function exportPdf(result) {
  ...
  doc.text(
    `검사 대상 문장 ${result.totalSentences}개 · 중복 그룹 ${result.groups.length}개 · 중복 발생 ${result.duplicateSentenceCount}회`,
    M,
    y
  );
  y += 9;
  ...
  if (!clean) {
    autoTable(doc, {
      ...
      body: result.groups.map((g, i) => [
        String(i + 1),
        g.sentence + (g.crossFile ? '\n[파일 간 중복]' : ''),
        String(g.count),
        g.occurrences.map((o) => `${o.file} · ${o.location}`).join('\n'),
      ]),
      styles: {
        font: 'Paperlogy',
        fontStyle: 'normal',
