...
// 명사형 종결('~함', '~임' 등)로 끝나는 글자 — 이런 경우는 실제 문장으로 본다.
const NOMINAL_ENDINGS = '함임음됨남봄줌옴짐움림듬듦름감냄다요';

/**
 * 제목·라벨 같은 서식 문구인지 판단한다.
 * 종결 부호가 전혀 없고, 길이가 짧고, 문장형 어미로 끝나지 않으면
 * (예: "행동특성 및 종합의견 기록사항", "2026학년도 진로활동 기록지")
 * 문서 서식으로 보고 중복 검사에서 제외한다.
 */
export function isLabelLike(sentence, key) {
  if (/[.!?…。]/.test(sentence)) return false;
  if (key.length >= 20) return false;
  const last = sentence.trim().slice(-1);
  return !NOMINAL_ENDINGS.includes(last);
}

const HAS_TERMINATOR = /[.!?…。]/;

/**
 * AI 검토가 필요한 경계 문구를 모은다 (정규화 키 기준으로 중복 제거).
 * - excludedByRule: 규칙이 서식으로 뺀 문구 → 실제 문장이면 되살려야 함
 * - uncertainIncluded: 규칙이 통과시켰지만 종결부호가 없는 짧은 문구
 *   (예: '이바다' 같은 이름) → 서식이면 빼야 함
 * 종결부호가 있는 문장은 확실한 문장으로 보고 검토 대상에서 제외한다.
 */
  ...
  for (const f of fileUnits) {
    for (const u of f.units) {
      for (const s of splitSentences(u.text)) {
        ...
        if (u.isHeader || isLabelLike(s, key)) {
          if (!excluded.has(key)) excluded.set(key, s);
        } else if (!HAS_TERMINATOR.test(s) && key.length < 25) {
          if (!uncertain.has(key)) uncertain.set(key, s);
        }
      }
    }
  }
  const toList = (m) => [...m].map(([key, text]) => ({ key, text }));
  return { excludedByRule: toList(excluded), uncertainIncluded: toList(uncertain) };
}
  ...
  for (const f of fileUnits) {
    ...
    for (const u of f.units) {
      for (const s of splitSentences(u.text)) {
        ...
        let excludedFlag = byHeader || isLabelLike(s, key);
