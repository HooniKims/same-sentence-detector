 ...
 * 마침표로 끝나지 않는 문구는 정의상 문장이 아니므로 검토하지 않는다.
 * 검토 대상은 머리글 행에서 나왔지만 마침표로 끝나는 텍스트뿐이다 —
...
export function collectReviewCandidates(fileUnits) {
  ...
  for (const f of fileUnits) {
    for (const u of f.units) {
      if (!u.isHeader) continue;
      for (const s of splitSentences(u.text)) {
        const key = normalizeKey(s);
        if (!isEligible(key, 2)) continue;
        if (isCompleteSentence(s) && !excluded.has(key)) excluded.set(key, s);
      }
    }
  }
