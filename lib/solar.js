...
/**
 * 중복으로 잡힌 문장이 온전한 서술인지, 잘못 찍힌 마침표로 잘린 조각인지 확인한다.
 * (예: "글감. 선택 과정에서…"의 "글감." 조각) 애매하면 '문장'으로 남겨
 * 진짜 중복을 놓치지 않는다.
 */
export function reviewDuplicateGroups(items) {
  return classifyItems(items, {
    system:
      '당신은 한국어 문장 분석기입니다. 텍스트가 완결된 문장인지, 문장 중간에 잘못 찍힌 마침표로 잘린 조각인지 판단합니다. 반드시 JSON 배열만 출력합니다.',
    intro: [
      '아래 텍스트는 모두 마침표로 끝나지만, 일부는 문장 중간에 잘못 찍힌 마침표 때문에 만들어진 조각입니다.',
      '',
      '판단 방법 — 마침표 바로 앞의 어절이 무엇인지 보세요.',
      ...
      '',
      '판단이 애매하면 "문장"으로 분류하세요. 진짜 중복 문장을 놓치는 것이 더 나쁜 오류입니다.',
    ],
  });
}
